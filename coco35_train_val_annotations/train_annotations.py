...
def create_table(): 
    ...
    try: 
        ...
        sql='''CREATE TABLE IF NOT EXISTS train_annotations(
		...
		# Print exception 
        print("Error while creating train_annotations table", error) 
    finally: 
...
def Insert_annotations(image_id,bbox,point_17,foot_p,left_p,right_p,segmentations): 
    try: 
        ...
        try:		 
            ...
            # print("bbox:" ,bbox)
            cursor.execute("INSERT INTO train_annotations(image_id, bbox,body_keypoints,foot_kpts,left_hand_kpts,right_hand_kpts,segmentations) VALUES (%s, %s,%s, %s, %s, %s, %s)", (image_id,bbox,point_17,foot_p,left_p,right_p,segmentations))
			# Commit the changes to the database 
            conn.commit() 
        except (Exception, psycopg2.DatabaseError) as error: 
            print("Error while inserting data in train_annotations table", error) 
...
def check_existence(image_id_to_insert):
    try:
        ...
        check_query = "SELECT image_id FROM train_annotations WHERE image_id = %s"
        conn, cursor = create_connection() 
...
# Delete_Table("train_annotations") 
