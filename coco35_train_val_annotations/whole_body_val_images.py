...
def create_table(): 
    ...
    try: 
        ...
        sql='''CREATE TABLE IF NOT EXISTS whole_body_val_images(
		...
		# Print exception 
        print("Error while creating whole_body_val_images table", error) 
    finally: 
...
def Insert_ID_Images(image_id,image_link): 
	try: 
		...
		try:		 
			...
			cursor.execute("INSERT INTO whole_body_val_images(image_id, image_url) VALUES (%s, %s)", (image_id, image_link))
			# Commit the changes to the database 
			conn.commit() 
		except (Exception, psycopg2.DatabaseError) as error: 
			print("Error while inserting data in whole_body_val_images table", error) 
		finally: 
			# Close the connection object 
			conn.close() 
	finally: 
...
def check_existence(image_id_to_insert):
    try:
        ...
        check_query = "SELECT image_id FROM whole_body_val_images WHERE image_id = %s"
        conn, cursor = create_connection() 
